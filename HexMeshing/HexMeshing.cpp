...
int main (int argc, char *argv[]) {
	...
	if (reader->IsFilePolyData()) {
		...
		cout << mesh.patches.size() << endl;
		vector<Edge> c;
		for (int i = 0; i < mesh.patches.size(); i++) {
			vector<Edge> edges = mesh.extractFacesFromPatch(mesh.patches[i]);
			c.insert(c.begin(), edges.begin(), edges.end());
		}
		cout << c.size() << endl;

		ofstream outputFile1;
		outputFile1.open("surface_division.vtk");
		outputFile1 << "# vtk DataFile Version 1.0" << endl;
		outputFile1 << "Unstructured Grid Example" << endl;
		outputFile1 << "ASCII" << endl;
		outputFile1 << "\nDATASET UNSTRUCTURED_GRID" << endl;
		outputFile1 << "POINTS " << mesh.vertices.size() << " float" << endl;
		for (int i = 0; i < mesh.vertices.size(); i++) {
			outputFile1 << mesh.vertices.at(i).x << " " << mesh.vertices.at(i).y << " " << mesh.vertices.at(i).z << endl;
		}
		
		int ncells = c.size();
		outputFile1 << "CELLS " << ncells << " " << ncells * 3 << endl;
		for (int i = 0; i < ncells; i++) {
			outputFile1 << "2 " << c.at(i).v1 << " " << c.at(i).v2 << endl;
		}

		outputFile1 << "CELL_TYPES " << ncells << endl;
		for (int i = 0; i < ncells; i++) {
			outputFile1 << "3" << endl;
		}
		//
		...
		// Volume Division
		vector<Edge> c2 = mesh.getBoundaryEdges();
		for (int i = 0; i < mesh.corner_edges.size(); i++) {
			...
			c2.emplace_back(id1, id2, c.size());
		}

		ofstream outputFile;
		outputFile.open("volume_division.vtk");
		...
		for (int i = 0; i < ncells; i++) {
			...
			outputFile << "2 " << c2.at(i).v1 << " " << c2.at(i).v2 << endl;
		}
